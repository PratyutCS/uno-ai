...
# HUMAN player code
    ...
    def play_rand1(self, deck):

        cardNumber = int(input("enter the card position : "))
        while True:
            if(cardNumber>=1 and cardNumber<=len(self.hand_play)):
                break;
            else:
                cardNumber = int(input("enter Valid the card position : "))
        card = self.hand_play[cardNumber-1]
        ...
        if (self.card_play.color == "WILD") or (self.card_play.value == "PL4"):
            cardColor = input("enter the card's color : \nRED\nYEL\nBLU\nGRE\n")
            self.card_play.color = cardColor
